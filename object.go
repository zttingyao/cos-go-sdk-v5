...
func (s *ObjectService) PutRename(ctx context.Context, name, sourceURL string, opt *ObjectPutRenameOptions) (*Response, error) {
	surl := strings.SplitN(sourceURL, "/", 2)
	if len(surl) < 2 {
		return nil, errors.New(fmt.Sprintf("x-cos-rename-source format error: #{sourceURL}"))
	}
	var u string
	u = fmt.Sprintf("%s/%s?rename", surl[0], encodeURIComponent(name))
	sendOpt := &sendOptions{
		uri:       u,
		method:    http.MethodPut,
		body:      nil,
